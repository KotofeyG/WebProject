...
public class MealDaoImpl implements MealDao {
    ...
    private static final String FIND_MEAL_BY_ID = "SELECT meals.id, title, image, meal_types.type, price, recipe, created, active FROM meals" +
            ...
            " WHERE meals.id=?";
    private static final String FIND_MEAL_IN_ANY_ORDER = "SELECT order_id FROM orders_have_meals WHERE meal_id=? LIMIT 1";
    ...
    private static final String FIND_MEAL_STATUS_BY_ID = "SELECT active FROM meals WHERE id=?";
    ...
    public boolean deleteEntitiesById(List<Long> idList) throws DaoException {
        try (Connection connection = connectionPool.getConnection();
             ...
             PreparedStatement checkStatement = connection.prepareStatement(FIND_MEAL_IN_ANY_ORDER)) {
            ...
            logger.log(Level.INFO, result ? "deleteEntities method was completed successfully. Meals with id were deleted"
                    : "Meals weren't deleted");
            return result;
        } catch (SQLException e) {
    ...
    @Override
    public boolean isMealAvailable(long mealId) throws DaoException {
        try (Connection connection = connectionPool.getConnection();
             PreparedStatement statement = connection.prepareStatement(FIND_MEAL_STATUS_BY_ID)) {
            statement.setLong(FIRST_PARAM_INDEX, mealId);
            ResultSet resultSet = statement.executeQuery();
            boolean result = false;
            if (resultSet.next()) {
                result = resultSet.getBoolean(MEAL_ACTIVE);
            }
            logger.log(Level.DEBUG, " isMealAvailable method was completed successfully. Result: " + result);
            return result;
        } catch (SQLException e) {
            logger.log(Level.ERROR, "Impossible to check availability of meal. Database access error:", e);
            throw new DaoException("Impossible to check availability of meal. Database access error:", e);
        }
    }
