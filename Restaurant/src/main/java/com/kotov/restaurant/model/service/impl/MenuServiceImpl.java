...
public class MenuServiceImpl implements MenuService {
    ...
    public boolean insertNewMenu(String title, String type, String[] mealIdArray) throws ServiceException {
        boolean result = false;
        try {
            if (MenuValidator.isTitleValid(title) && MenuValidator.isTypeValid(type) && !menuDao.isMenuExist(title)) {
                ...
                if (mealIdList.isEmpty()) {
                    result = menuDao.insertNewEntity(menu) != 0;
                } else {
                    result = menuDao.insertNewEntity(menu, mealIdList) != 0;
                }
            }
    ...
    @Override
    public boolean updateMenuTitle(String menuIdStr, String title) throws ServiceException {
        boolean result = false;
        try {
            if (MenuValidator.isTitleValid(title) && !menuDao.isMenuExist(title)) {
                long menuId = Long.parseLong(menuIdStr);
                result = menuDao.updateMenuTitle(menuId, title);
            }
        } catch (DaoException e) {
            logger.log(Level.ERROR, "Menu title cannot be updated:", e);
            throw new ServiceException("Menu title cannot be updated:", e);
        } catch (NumberFormatException e) {
            logger.log(Level.WARN, "String menuIdStr does not contain a parsable long: " + menuIdStr);
        }
        return result;
    }
    ...
    public boolean addMealToUserCart(long userId, String mealIdStr, String mealQuantityStr) throws ServiceException {
        ...
        try {
            ...
            if (mealDao.isMealAvailable(mealId)) {
    ...
    @Override
    public int getAvailableMealCountForMenu(long menuId) throws ServiceException {
        try {
            return menuDao.getAvailableMealCountForMenu(menuId);
        } catch (DaoException e) {
    ...
    @Override
    public List<Meal> findAvailableMealsForMenu(long menuId, int page) throws ServiceException {
        try {
            return menuDao.findAvailableMealsForMenu(menuId, page);
        } catch (DaoException e) {
    ...
    public boolean addMealToMenu(String menuIdStr, String mealIdStr) throws ServiceException {
        ...
        try {
            long menuId = Long.parseLong(menuIdStr);
            long mealId = Long.parseLong(mealIdStr);
            result = menuDao.insertMealToMenu(menuId, mealId);
        } catch (DaoException e) {
