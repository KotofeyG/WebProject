...
<fmt:message key="menu_update.action_positive_result" var="action_positive_result"/>
<fmt:message key="menu_update.action_negative_result" var="action_negative_result"/>
<fmt:message key="menu_update.title_change_positive_result" var="title_change_positive_result"/>
<fmt:message key="menu_update.title_change_negative_result" var="title_change_negative_result"/>
<fmt:message key="meal_management.empty_picture" var="empty_picture"/>

<jsp:useBean id="marked_meals" scope="session" type="java.util.Map"/>
...
<div class="container-fluid">
    <c:choose>
        <c:when test="${action_result eq 'true'}">
            <div class="alert alert-success" id="message"><b class="valid_message">${action_positive_result}</b></div>
        </c:when>
        <c:when test="${action_result eq 'false'}">
            <div class="alert alert-warning long-message" id="message"><b class="invalid_message">${action_negative_result}</b></div>
        </c:when>
        <c:when test="${menu_title_change_result eq 'true'}">
            <div class="alert alert-success" id="message"><b class="valid_message">${title_change_positive_result}</b></div>
        </c:when>
        <c:when test="${menu_title_change_result eq 'false'}">
            <div class="alert alert-warning long-message" id="message"><b class="invalid_message">${title_change_negative_result}</b></div>
        </c:when>
    </c:choose>
    <div class="scroll-table">
        <table class="table-condensed table-bordered meal-table">
            ...
            <tr>
                ...
                <td colspan="2"><b>${menu_type} ${sessionScope.selected_menu.type.value}</b></td>
            </tr>
            <tr>
                <th>${meal_id}</th>
                <th>${meal_title}</th>
                    ...
                    <tr>
                        ...
                        <td>
                            <form action="controller" method="post">
                                <input type="hidden" name="command" value="update_menu_meals_command">
                                <input type="hidden" name="selected" value="${entry.key.id}"/>
                                <input type="hidden" name="selected_menu" value="${sessionScope.selected_menu.id}"/>
                                <c:choose>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="controller" method="post" id="menu-title-form" class="form-inline">
                        <input type="hidden" name="command" value="update_menu_title_command">
