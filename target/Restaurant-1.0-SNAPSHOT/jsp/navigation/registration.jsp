...
<div class="container-fluid" id="container-fluid">
    <form action="${abs}/controller" method="post">
        ...
        <c:choose>
            <c:when test="${invalid_login eq 'invalid_message'}"><b>${invalid_login_message}</b></c:when>
            <c:when test="${invalid_login eq 'not_unique_message'}"><b>${not_unique_login_message}</b></c:when>
        </c:choose>
        <br/><br/>
        <label for="password">${psw}</label><br/>
        <input type="password" id="password" class="form-control" name="password" placeholder="${psw}">
        <c:if test="${invalid_passport eq 'invalid_message'}"><b>${invalid_psw_message}</b></c:if>
        <br/><br/>
        ...
        <c:if test="${invalid_passport eq 'password_mismatch'}"><b>${psw_mismatch_message}</b></c:if>
        <br/><br/>
        ...
        <c:choose>
            <c:when test="${invalid_email eq 'invalid_message'}"><b>${invalid_email_message}</b></c:when>
            <c:when test="${invalid_email eq 'not_unique_message'}"><b>${not_unique_email_message}</b></c:when>
        </c:choose>
        <br/><br/>
        ...
        <c:choose>
            <c:when test="${invalid_mobile_number eq 'invalid_message'}"><b>${invalid_mobile_number_message}</b></c:when>
            <c:when test="${invalid_mobile_number eq 'not_unique_message'}"><b>${not_unique_mobile_number_message}</b></c:when>
        </c:choose>
        <br/><br/>
